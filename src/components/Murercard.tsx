...
export default function Murercard({ blok }: { blok: any }) {
  ...
  const image = (
    <div className={`relative mx-auto w-full max-w-[385px] ${imageSide === "right" ? "md:mr-0" : "md:ml-0"}`}>
      <div className={`absolute top-7 h-full w-full rounded bg-tertiary ${imageSide === "right" ? "left-7" : "-left-7"}`} />
      {blok.image?.filename && <img src={blok.image.filename} alt={blok.image.alt || blok.title || ""} className="relative z-10 aspect-square w-full object-cover" />}
    </div>
  );
